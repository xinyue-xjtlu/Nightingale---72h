...
def create_app(site_role):
    ...
    # ---------- doctor site ----------
        ...
        def ai_suggest(patient_id):
            """Regenerate the AI suggestion with the real LLM (ai.py).
            Falls back to the static simulated text when the API is not
            ...
            try:
                suggestion = ai.generate_suggestion(patient)
            except Exception:
                suggestion = None
            if suggestion:
                patient['ai_suggestion'] = suggestion
                patient['ai_source'] = (f"generated by {ai.MODEL} · patient "
                                        "text redacted before sending")
            else:
                patient['ai_source'] = ("simulated fallback — set "
                                        "NIGHTINGALE_LLM_API_KEY to enable a real LLM")
            return redirect('/doctor')
