...
- **AI suggestion with optional real LLM** — each patient carries an
  `ai_suggestion` field. By default it is static simulated text; setting
  one environment variable switches it to a real LLM (DeepSeek by default,
  any OpenAI-compatible API works). Patient text is PHI-redacted before it
  leaves the machine, and any failure falls back to the static text.
...
```
nightingale/
├── app.py              # three-site Flask app: routes, edit logic, versioning
├── ai.py               # optional real-LLM integration (DeepSeek default) + PHI redaction
...
└── test/
    ├── test_rbac_scope.py        # 10 role-permission tests
    ├── test_concurrent_edits.py  # 8 concurrent-edit tests
    └── test_ai.py                # 6 AI-integration tests
```
...
`test/test_rbac_scope.py` (10 tests: per-site 404s, wrong-claim rejection
...
Covered by `test/test_concurrent_edits.py` (8 tests), including the
cross-role unseen-change flow.

## Data & privacy

- **Synthetic data only** — Tom and Tina are fictional.
- **LLM is off by default**: without `NIGHTINGALE_LLM_API_KEY` no patient
  data ever leaves the machine (the suggestion is static simulated text).
- **When the LLM is enabled**, `ai.py` redacts names / NRICs / phone
  numbers from the patient text before the request goes out, and any
  failure falls back to the static text. See "AI integration" above.
- Session state is signed server-side via Flask's `secret_key`.

## AI integration (optional, one env var to enable)

The doctor page has a **"regenerate with AI"** link next to each patient's
AI Suggestion. The call chain lives in `ai.py`:

1. The patient's story and diagnosis are **PHI-redacted** (names, NRICs,
   phone numbers) before anything leaves the machine.
2. A chat-completion request goes to the configured provider.
3. On success the suggestion is stored with a source label
   ("generated by deepseek-chat · patient text redacted before sending");
   on any failure (or when unconfigured) the static simulated text stays,
   labelled as fallback — the demo never breaks.

Configuration (all read from environment variables, defaults = DeepSeek):

| Variable | Default | Meaning |
|---|---|---|
| `NIGHTINGALE_LLM_API_KEY` | *(empty — LLM disabled)* | your API key; set it to enable the LLM |
| `NIGHTINGALE_LLM_BASE_URL` | `https://api.deepseek.com/v1` | any OpenAI-compatible endpoint |
| `NIGHTINGALE_LLM_MODEL` | `deepseek-chat` | model name at that provider |

Swapping provider = changing these three values. No new dependencies
(`ai.py` uses only the standard library).

```bash
NIGHTINGALE_LLM_API_KEY=sk-... python3 app.py     # mac/linux
# or: export NIGHTINGALE_LLM_API_KEY=sk-... && python3 app.py
```

Never commit the key — it is read from the environment only, and `ai.py`
...
- `test_rbac_scope.py` — role permission matrix: per-site 404s, wrong-claim
  rejection, patient identity binding, simultaneous three-role sessions.
- `test_concurrent_edits.py` — field independence, version bumps and
  history appends, identical/empty edits ignored, the cross-role
  unseen-change detection flow, and cross-site visibility of edits.
- `test_ai.py` — PHI redaction rules, graceful fallback without an API key,
  success/failure paths of the regenerate flow (mocked — no network), and
  route gating.
...
- Same-field concurrent edits are last-write-wins without an explicit
  conflict prompt.
- The LLM redaction is regex-based (names/NRICs/phones only) — a fuller
  production pipeline would add address, DOB and provider-name patterns.
- LLM responses are not cached or audited yet; each regenerate costs one
  API call.
